...
export default function ClientPortalPage({ portalCode }) {
  const portalQuery = new URLSearchParams(window.location.search);
  const featuredContentId = portalQuery.get('content') || '';
  const isSuperAdminImpersonating = portalQuery.get('superadmin') === '1';
  ...
  const [activeTab, setActiveTab] = useState(
    portalQuery.get('tab') === 'content' ? 'content' : 'tasks'
  ); // 'tasks' | 'content' | 'insights' | 'breathing' | 'clinic'
  ...
  const handlePreviewSelfContentUrl = async (urlToPreview) => {
