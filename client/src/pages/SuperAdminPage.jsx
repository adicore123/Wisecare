...
// Professional Titles in Therapy and Clinical Practice (Categorized)
const THERAPIST_PROFESSIONAL_TITLES = [
  ...
  {
    category: 'מקצועות הבריאות ושיקום',
    titles: [
      ...
      'מאמן/ת ומטפל/ת קוגניטיבי (Coaching)',
...
const THERAPY_SPECIALTIES_SUGGESTIONS = [
  'חרדה וטראומה (PTSD)',
  'דיכאון ומצבי משבר',
...
הנהלת המערכת WiseCare`,
  ...
  const getTherapistLoginUrl = (loginCode) => {
    return `${window.location.origin}/login/${loginCode}`;
  };
  ...
  const handleSendWhatsAppInvite = async (therapist) => {
    try {
      ...
      const res = await api.sendTherapistInviteWhatsApp(currentAdmin.id, therapist.id);
      if (res.sentViaGreenApi) {
        showToast(`פרטי ההתחברות והקישור נשלחו בהצלחה לוואטסאפ של ${therapist.name}! 🚀`);
      } else {
        const text = encodeURIComponent(res.message);
        const cleanPhone = therapist.phone ? therapist.phone.replace(/[^0-9]/g, '') : '';
        const phoneFormatted = cleanPhone.startsWith('0') ? '972' + cleanPhone.slice(1) : cleanPhone;
        window.open(`https://wa.me/${phoneFormatted}?text=${text}`, '_blank');
        showToast(`נפתח WhatsApp לשליחת פרטי ההתחברות למטפל/ת`);
      }
    } catch (err) {
  ...
  const loadData = async () => {
    try {
      ...
      const [statsRes, therapistsRes, settingsRes, clientsRes] = await Promise.all([
        api.getSuperadminStats(currentAdmin.id),
        api.getTherapists(currentAdmin.id),
        api.getSuperadminSettings(currentAdmin.id).catch(() => null),
  ...
  const handleSaveSettings = async (e) => {
    ...
    try {
      ...
      const res = await api.updateSuperadminSettings(currentAdmin.id, superadminSettings);
      if (res.settings) {
  ...
  const handleRefreshGreenApiStatus = async () => {
    try {
      const res = await api.getSuperadminSettings(currentAdmin.id);
  ...
  const handleCreateTherapist = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await api.createTherapist(currentAdmin.id, {
        ...formData,
  ...
  const handleResetCredentials = async (e) => {
    ...
    try {
      ...
      const res = await api.resetTherapistCredentials(currentAdmin.id, resetModalTherapist.id, {
  ...
  const handleToggleActive = async (therapist) => {
    try {
      await api.updateTherapist(currentAdmin.id, therapist.id, {
                  ...
                  {filtered.map(therapist => (
                    <tr key={therapist.id}>
                      ...
                      <td style={{ textAlign: 'left' }}>
                        <button
                          onClick={() => onImpersonateTherapist(therapist)}
                          className="btn btn-primary"
                          style={{
                            padding: '6px 14px',
                            fontSize: '0.82rem',
            ...
            <div className="modal-card" style={{ maxWidth: '640px' }} onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleCreateTherapist} autoComplete="off">
                ...
                <div className="modal-body" style={{ padding: '20px 24px', maxHeight: '72vh', overflowY: 'auto' }}>
                  ...
                  {/* Row: Professional Title (Combobox) & Specialties */}
                  <div className="form-row" style={{ marginBottom: '14px' }}>
                    ...
                    <div className="form-group" style={{ marginBottom: 0, position: 'relative' }} ref={titleDropdownRef}>
                      ...
                      <div style={{ position: 'relative' }}>
                        <input
                          type="text"
                          className="form-control"
                          list="therapist-titles-datalist"
                          autoComplete="off"
                      ...
                      <datalist id="therapist-titles-datalist">
                        {ALL_TITLES_FLAT.map(t => (
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '6px' }}>
                        {['פסיכולוג/ית קליני/ת', 'עו"ס קליני/ת (MSW)', 'פסיכותרפיסט/ית CBT', 'מטפל/ת באמנות'].map(chip => (
                          <button
                            key={chip}
                            type="button"
                            onClick={() => {
                    ...
                    <div className="form-group" style={{ marginBottom: 0 }}>
                      <div style={{ height: '26px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                        ...
                        <span style={{ fontSize: '0.72rem', color: '#64748b' }}>מופרדים בפסיקים</span>
                      </div>

                      <input
                        type="text"
                        className="form-control"
                        list="therapist-specialties-datalist"
                        placeholder="לדוגמה: חרדה, טראומה, CBT, ילדים..."
                        value={formData.specialty}
                        onChange={e => setFormData({ ...formData, specialty: e.target.value })}
                      />

                      <datalist id="therapist-specialties-datalist">
                        {THERAPY_SPECIALTIES_SUGGESTIONS.map(s => (
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '6px' }}>
                        {['חרדה וטראומה', 'דיכאון ומשבר', 'ילדים ונוער', 'טיפול זוגי'].map(chip => (
                          <button
                            key={chip}
                            type="button"
                            onClick={() => {
                              const current = formData.specialty ? formData.specialty.split(',').map(s => s.trim()) : [];
                              if (!current.includes(chip)) {
                                const next = [...current.filter(Boolean), chip].join(', ');
                        ...
                        <div style={{ position: 'relative' }}>
                          <input
                            ...
                            style={{
                              ...
                              WebkitTextSecurity: showAddPassword ? 'none' : 'disc',
                              minHeight: '44px'
                            }}
                          />
                          <button
      ...
      {newTherapistSuccess && (
        <div className="modal-overlay" onClick={() => setNewTherapistSuccess(null)}>
          <div className="modal-card" style={{ maxWidth: '520px' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer" style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', borderTop: '1px solid #e2e8f0', paddingTop: '14px' }}>
              <button
                type="button"
                className="btn btn-secondary"
                  ...
                  <div style={{ position: 'relative' }}>
                    <input
                      ...
                      style={{
                        ...
                        WebkitTextSecurity: showResetPassword ? 'none' : 'disc'
                      }}
                    />
                    <button
                      type="button"
