...
/**
 * Scan appointments and send automatic 24h reminders (called by Vercel Cron or background task)
 */
  ...
  try {
    ...
    for (const apt of allAppointments) {
      ...
      try {
        ...
        summary.errors.push(`Apt ${apt.id}: ${err.message}`);
      }
    }
  } catch (outerErr: any) {
    summary.errors.push(`Scheduler error: ${outerErr.message}`);
  }
  return summary;
}
